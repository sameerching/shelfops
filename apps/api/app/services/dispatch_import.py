...
from app.models.entities import Brand, DispatchRecord, ImportBatch, ImportRowError, SKU
from app.schemas.imports import ImportSummaryResponse, RowErrorSchema

REQUIRED_COLUMNS = {"sku_code", "platform", "city", "dispatch_qty", "dispatch_status", "dispatch_date"}


def parse_dispatch_file(filename: str, data: bytes) -> pd.DataFrame:
...
def _parse_qty(value: object) -> Decimal:
    if value is None or pd.isna(value):
        raise ValueError("dispatch_qty is required")
    try:
...
def _parse_date(value: object):
    text = _required_text(value, "dispatch_date")
    if re.fullmatch(r"[+-]?\d+(\.\d+)?", text):
        ...
        raise ValueError("Invalid date value for dispatch_date")
...
def import_dispatch_report(db: Session, brand_id: int, file_name: str, content: bytes) -> ImportSummaryResponse:
    brand = db.get(Brand, brand_id)
    if brand is None:
        raise HTTPException(
            status_code=status.HTTP_400_BAD_REQUEST,
            detail=f"Brand {brand_id} does not exist. Create the brand before importing dispatch tracker.",
        )

    batch = ImportBatch(brand_id=brand_id, file_type="dispatch_tracker", file_name=file_name, status="processing")
    db.add(batch)
    db.flush()

    errors: list[ImportRowError] = []
    df = parse_dispatch_file(file_name, content)
    ...
    for row_idx, row in df.iterrows():
        ...
        try:
            ...
            dispatch_qty = _parse_qty(row.get("dispatch_qty"))
            dispatch_status = _required_text(row.get("dispatch_status"), "dispatch_status")
            ...
            elif message == "Invalid numeric value for dispatch_qty":
                column_name = "dispatch_qty"
                error_code = "invalid_numeric"
            elif message == "dispatch_qty cannot be negative":
                column_name = "dispatch_qty"
                error_code = "negative_value"
            elif message == "Invalid date value for dispatch_date":
