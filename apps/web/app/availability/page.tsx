...
export default function AvailabilityPage() {
  ...
  const onFetch = async (event: FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      const data: AvailabilityRow[] = await response.json();
      setRows(data);
    } catch (fetchError) {
  ...
  return (
    <main className="mx-auto flex min-h-screen max-w-6xl flex-col gap-6 px-6 py-10">
      <h1 className="text-3xl font-semibold">Availability Snapshots</h1>
          ...
          <tbody>
            {rows.map((row) => (
              <tr key={row.id} className="border-t border-slate-200">
                ...
                <td className="px-3 py-2">{row.location.platform}</td>
                <td className="px-3 py-2">{row.location.city}</td>
                <td className="px-3 py-2">{row.location.location}</td>
              </tr>
            ))}
            {rows.length === 0 && (
              <tr>
                <td className="px-3 py-4 text-slate-500" colSpan={6}>
                  No availability snapshots loaded.
                </td>
              </tr>
            )}
          </tbody>
